    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      {step === 3 && (
        <>
          ...
          <div className="flex items-start gap-3 mt-2 p-4 rounded-xl border border-border bg-white/5 dark:bg-black/20">
            <input 
              type="checkbox" 
              id="lgpd" 
              checked={acceptedTerms}
              onChange={(e) => setAcceptedTerms(e.target.checked)}
              className="mt-1 w-4 h-4 rounded border-border text-red-600 focus:ring-red-600 focus:ring-offset-black"
            />
